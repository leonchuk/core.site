...
define('DPHP_USE_ACCESSIBLE', (float) phpversion() > 5.3);
class Dumphper
{
  /** config **/

  #static $encoding = 'UTF-8';  /** Text encoding, needed to escape stings **/
  static $default_escape_values = true;
  ...
  static function _dump(&$source, &$parents = array())
  {
    ...
    switch(gettype($source))
    {
      case 'array':
        ...
        break;
      case 'object':
        if(method_exists($source, '__sleep')) $source->__sleep();
      ...
      case 'resource':
        ...
        break;
      default:
        self::drawScalar($source, self::$default_escape_values);
        break;
    }
    self::$depth--;
  }
  ...
  {
    return ( is_string($source) ? ( $source ? htmlspecialchars($source) : ' ') : $source );
  }
  ...
  static function drawObject(&$source, &$parents)
  {
    ...
    if($className=='SimpleXMLElement') $source=(object)(array)$source;
...
.dumphper-container .dumphper-container { display: block !important; position: relative !important; }
...
.dumphper-value { padding: 0 3px 0 12px !important; background-repeat: no-repeat !important; cursor: default !important;max-height:100px; overflow-x:visible; overflow-y:auto; white-space:pre;}
